...
**AI Guest Agent** — an automated vacation rental guest messaging system. An AI agent receives guest messages from booking platforms (Airbnb, Booking.com, Vrbo) via Hospitable, classifies them, and either answers from a knowledge base, creates maintenance tickets, processes extra requests through Turno, or escalates to a human.

The full specification lives in `master-execution-plan.html` (Version 3.0). Refer to it for detailed API endpoints, database schema, workflow steps, and prompt templates.

## Tech Stack

- **Orchestration:** Trigger.dev (3 workflows)
...
The system uses a **Coordinator → Sub-Workflow** pattern inside a single Trigger.dev workflow:

1. **Coordinator (AI Step #1)** — An agentic loop with tool-use. Classifies the guest message and calls one of four tools.
2. Each tool maps to a **hard-coded sub-workflow** (dedicated function with fixed logic, DB queries, API calls, and optionally its own AI call):
   - **Sub-Workflow A** (`use_knowledge_base`) — Loads KB from Supabase, calls KB Answerer AI (#2). If no answer → triggers D (hard stop).
   - **Sub-Workflow B** (`raise_maintenance_ticket`) — Calls Urgency Assessor AI (#3), creates ticket in Supabase, sends SMS alerts.
   - **Sub-Workflow C** (`process_extra_request`) — Checks `allowed_extras` table, creates Turno task if approved, sends SMS if declined.
   - **Sub-Workflow D** (`escalate_to_human`) — **HARD STOP.** Logs KB gap, sets 8hr cooldown, sends SMS. No reply to guest. Workflow terminates.

The agent composes and sends the final guest reply — sub-workflows only perform actions and report back. Exception: Sub-Workflow D terminates without returning to the agent.

## Critical Rules

- **Sub-Workflow D = no reply to guest.** The AI must never guess, placeholder, or respond when escalating. Silence.
- **Cooldowns are per-property**, not per-guest. When active, ALL messages for that property are ignored.
- **Host messages must be filtered out** — the webhook fires for both guest and host messages.
- **Hospitable rate limits:** 2 messages/min per reservation, 50/5min globally.

## Database Tables (Supabase)

`properties`, `knowledge_bases`, `kb_gap_log`, `cooldowns`, `maintenance_tickets`, `extra_requests`, `sms_recipients`, `urgency_categories`, `allowed_extras`. Auth managed by Supabase Auth. RLS enabled on all tables (authenticated = full access, anon = blocked).

## Three Trigger.dev Workflows

1. **Main Agent Workflow** — HTTP POST trigger from Modal. Processes guest messages through the AI agent loop.
2. **Property Sync Workflow** — Manual trigger from dashboard. Pulls properties from Hospitable + Turno, upserts into Supabase.
3. **Supabase Keep-Alive** — Daily scheduled trigger. Prevents free-tier project pausing.
