      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {isSuperAdmin && (
          <button
            ...
            disabled={syncing}
            className="flex items-center gap-1.5 px-3 py-1 text-[11px] font-medium bg-gray-900 text-white rounded hover:bg-gray-800 disabled:opacity-50"
          >
        ...
        <table className="w-full text-[12px]">
          <thead>
            <tr className="border-b border-gray-100 text-[10px] text-gray-400 uppercase tracking-wider">
              ...
              <th className="text-left px-4 py-2 font-medium">Hospitable ID</th>
              <th className="text-left px-4 py-2 font-medium">Turno ID</th>
              <th className="text-left px-4 py-2 font-medium">Status</th>
              {isSuperAdmin && <th className="px-4 py-2 font-medium" />}
            </tr>
          </thead>
          <tbody>
            {properties.map((p) => (
              <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                ...
                <td className="px-4 py-2 text-gray-400 font-mono text-[10px]">{p.hospitable_property_uuid.slice(0, 8)}...</td>
                <td className="px-4 py-2 text-gray-400">{p.turno_property_id || "—"}</td>
                <td className="px-4 py-2">
                  <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${p.is_active ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-400"}`}>
                    {p.is_active ? "Active" : "Inactive"}
                  </span>
                </td>
                {isSuperAdmin && (
                  <td className="px-4 py-2 text-right">
                    <button onClick={() => toggleActive(p.id, p.is_active)} className="text-[11px] text-gray-400 hover:text-gray-600">
            ...
            {properties.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400 text-[11px]">No properties yet. Run a sync to import from Hospitable.</td></tr>
            )}
          </tbody>
        </table>
      </div>
