    ...
    <div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
        ...
        <div className="flex gap-4 items-end">
          ...
          <button onClick={invite} disabled={inviting}
            className="flex items-center gap-2 px-5 py-2 text-base font-medium bg-gray-900 text-white rounded-lg hover:bg-gray-800 disabled:opacity-50 shrink-0">
            <Plus size={20} /> {inviting ? "..." : "Invite"}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 text-sm text-gray-400 uppercase tracking-wider font-medium">Users</div>
          {users.map((u) => (
            <div key={u.id} onClick={() => setSelectedUser(u.id)}
              className={`flex items-center justify-between px-5 py-4 cursor-pointer border-b border-gray-50 ${selectedUser === u.id ? "bg-gray-50" : "hover:bg-gray-50/50"}`}>
              <div>
                <div className="text-base text-gray-900">{u.email}</div>
                <div className="text-sm text-gray-400">{u.full_name || "No name"}</div>
              </div>
              <div className="flex items-center gap-2.5">
                <span className={`text-sm px-3 py-0.5 rounded-lg font-medium ${u.role === "super_admin" ? "bg-purple-50 text-purple-700" : "bg-gray-100 text-gray-500"}`}>
                  {u.role === "super_admin" ? "Admin" : "Member"}
                </span>
                <button onClick={(e) => { e.stopPropagation(); toggleRole(u.id, u.role); }} title="Toggle role"
                  className="p-2 text-gray-300 hover:text-purple-500"><Shield size={20} /></button>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Property access */}
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 text-sm text-gray-400 uppercase tracking-wider font-medium">
            Property Access {selectedUser ? `— ${users.find((u) => u.id === selectedUser)?.email}` : ""}
          </div>
          {!selectedUser ? (
            <div className="px-5 py-10 text-center text-base text-gray-400">Select a user to manage access</div>
          ) : (
            <>
              <div className="px-5 py-4 border-b border-gray-50">
                <select onChange={(e) => { if (e.target.value) addAccess(e.target.value); e.target.value = ""; }}
                  className="w-full px-4 py-2 text-base border border-gray-200 rounded-lg bg-white">
              ...
              {userAccess.map((a) => (
                <div key={a.id} className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
                  <div className="flex items-center gap-2.5 text-base text-gray-700">
                    <Building2 size={20} className="text-gray-400" /> {(a.properties as any)?.name}
                  </div>
                  <button onClick={() => removeAccess(a.id)} className="p-2 text-gray-300 hover:text-red-500"><Trash2 size={20} /></button>
                </div>
              ))}
              {userAccess.length === 0 && (
                <div className="px-5 py-8 text-center text-base text-gray-400">No properties assigned. Admins have access to all.</div>
              )}
            </>
          )}
        </div>
